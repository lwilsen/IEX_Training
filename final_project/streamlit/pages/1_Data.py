...
- You can see a slight slope in both lines (indicating better performance), but not by a practically significant amount""")
    ...
    - We can see that the most important predictors are:
        ...
        - Exter Qual
        - Overall Qual
    - Here, an **increase by 1** in any of the parameters causes an **increase by one hundred thousand dollars** in saleprice.
...
def movies(df):
    ...
    st.text_area("Processed Review", [word for word in tokenized_review if word not in stop], height=200)
    st.divider()
    st.write('**Models**')
    st.write("""
I used two models:
...
- **What It Does**: This function is like a guide that helps the model learn to make better predictions. It compares the model’s guesses with the correct answers (the target labels). The `from_logits=True` part simply means that the model's raw outputs (logits) are automatically turned into probabilities before this function does its job.
""")

    st.write("""
### 3. Evaluation Metric: Accuracy
- **What It Does**: Accuracy tells us how well the model is performing by calculating the percentage of correctly guessed images. If the model's predictions match the correct labels, it gets points for accuracy, and we use this to judge how well the model is doing overall.
""")
...
data_option = st.radio("Select Data:", 
                        ("Housing","Titanic", "Movie", "MNIST"),
                        key = "data_option")
