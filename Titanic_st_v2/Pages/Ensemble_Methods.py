...
st.write("## Bagging")
bagging_model = BaggingClassifier(estimator=pipeline_lr, n_estimators=10, random_state=42)
...
stacking_model = StackingClassifier(estimators=level1_models, final_estimator=final_estimator, cv=5)
...
voting_model = VotingClassifier(estimators=level1_models, voting='hard')  # Hard voting for classification - SOFT is regression
...
st.write(f"**The overall best model was the: {max(models)} model with an accuracy of: {models[max(models)]}**")