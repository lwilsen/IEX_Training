...
def predict_func(
    mod, scaler, user_params, x_train_features=x_train, y_train_labels=y_train
):
...
"""
    ...
    st.image(
        "/Users/lukewilsen/Desktop/IEX/IEX_Training/Titanic/titanic.jpeg",
        caption="Titanic",
...
def exploration(y_train=y_train, x_train=x_train, train=train):
    ...
    st.write(
        "This feature selection was important to my analysis because it helped me to determine which variables I needed to pay the most attention to"
    )
    importance_list = []
    ...
    importance_df = pd.concat(
        [pd.DataFrame([x]) for x in importance_list], ignore_index=True
    )
    ...
    df_pie["Class"] = (
        df_pie[["class_1", "class_2", "class_3"]].idxmax(axis=1).str.split("_").str[1]
    )
    df_pie = df_pie.drop(["class_1", "class_2", "class_3"], axis=1)

    fig = px.pie(
        df_pie, names="Class", values="Survived", title="Survivor Breakdown by Class"
    )
...
- Majority of passengers were 3rd class; explains why % of survivors in 1st class roughly equals % of survivors in 3rd class
    ...
    fig = px.pie(
        ...
        names="Male",
        values="Survived",
        title="Survivor Breakdown by Gender",
        color="Male",
    ...
    fig = px.pie(
        train,
        names="Male",
        title="Passenger Breakdown by Gender",
        color="Male",
        color_discrete_map={0: "lightblue", 1: "darkblue"},
    )
    st.plotly_chart(fig)
...
def models():
    ...
    svm_PR_disp = PrecisionRecallDisplay.from_estimator(
        svm_mod, x_test_standard, y_test
    )
    ...
    svc_PR_disp = PrecisionRecallDisplay.from_estimator(
        svc_rbf_mod, x_test_standard, y_test
    )
    ...
    pipeline = make_pipeline(
        standard_scaler, SVC(kernel="rbf", C=1, random_state=1, gamma=0.1)
    )
...
def prediction():
    ...
    pipeline = Pipeline(
        [("Scaler", StandardScaler()), ("LR", LogisticRegression(**best_params))]
    )
    pipeline.fit(x_train, y_train)
    ...
    if pipeline.predict(df) == 1:
        ...
        st.image(
            "/Users/lukewilsen/Desktop/IEX/IEX_Training/Titanic/happy_sailor.jpg",
            use_column_width=False,
        )
    else:
        st.write("Uh Oh.")
        st.image(
            "/Users/lukewilsen/Desktop/IEX/IEX_Training/Titanic/you_died.png",
            use_column_width=False,
        )
